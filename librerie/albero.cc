...
Albero init_albero() { return nullptr; }
...
void insert(Albero & a, int value) {
  if (a == nullptr) { // caso base se l'albero è vuoto lo inizializzo con un
    ...
    a = init_albero(value);
  } else if (value < a->value) { // se il valore è minore del valore del nodo
                                 // corrente vado a sinistra
...
bool search(const Albero a, int value) {
  if (a == nullptr) { // caso base se l'albero è vuoto non ho trovato il valore
    return false;
             ...
             value) { // caso base se il valore del nodo corrente è uguale al
                      // valore che sto cercando ho trovato il valore
    return true;
  } else if (value < a->value) { // se il valore è minore del valore del nodo
                                 // corrente vado a sinistra
