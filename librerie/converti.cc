...
int converti_base_ricorsivo(int numero, int base_partenza, int base_arrivo) {
    ...
    return (numero % base_arrivo) + converti_base_ricorsivo(numero / base_arrivo, base_partenza, base_arrivo) * base_partenza;
}
...
char * converti_base_str(int numero, int base_partenza, int base_arrivo) {
    ...
    char * risultato = new char[length]; // Allocate memory for the risultato array with the exact length
    int i = 0;
    while (numero > 0) {
...
int main() {
    ...
    cin >> base_arrivo;
    char* risultato = converti_base_str(numero, base_partenza, base_arrivo); // Store the returned risultato array
    cout << "Il numero " << numero << " in base " << base_partenza << " e' uguale a " << risultato << " in base " << base_arrivo << endl;
