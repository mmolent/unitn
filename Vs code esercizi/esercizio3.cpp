...
int conta(int val, list* lista, int contatore)
{
  if(lista==NULL) return contatore;

  else return conta(val, lista->next, contatore + (lista->code==val));
}
...
list* estrai(list* dipendenti, list* entrate, list* uscite)
{
  ...
  while(dipendenti!=NULL)
    {
      if(conta(dipendenti->code, entrate, 0)==conta(dipendenti->code, uscite, 0))
